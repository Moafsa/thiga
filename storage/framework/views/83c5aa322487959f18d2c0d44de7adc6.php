<?php $__env->startSection('title', 'Meu Perfil - TMS Motorista'); ?>
...
<div class="profile-card">
    <div class="profile-header">
        <div class="profile-photo-container">
            ...
            ?>
            <?php if($hasRealPhoto && $photoUrl): ?>
                <img src="<?php echo e($photoUrl); ?>" alt="Foto do Perfil" class="profile-photo" id="profile-photo-preview" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
        ...
        <h2 style="color: var(--cor-acento); margin-bottom: 5px;"><?php echo e($driver->name); ?></h2>
        <p style="color: rgba(245, 245, 245, 0.7); font-size: 0.9em;"><?php echo e($driver->email); ?></p>
    </div>
    ...
    <form action="<?php echo e(route('driver.profile.update')); ?>" method="POST" enctype="multipart/form-data" id="profile-form">
        <?php echo csrf_field(); ?>
        ...
        <div class="form-group">
            ...
            <input type="text" id="name" name="name" value="<?php echo e(old('name', $driver->name)); ?>" required>
        </div>
        ...
        <div class="form-group">
            ...
            <input type="email" id="email" value="<?php echo e($driver->email); ?>" disabled>
        ...
        <div class="form-group">
            ...
            <input type="tel" id="phone" name="phone" value="<?php echo e(old('phone', $driver->phone)); ?>" placeholder="(00) 00000-0000">
        ...
        <div style="border-top: 1px solid rgba(255, 255, 255, 0.1); margin: 30px 0; padding-top: 30px;">
            ...
            <div class="form-group">
                ...
                <input type="text" id="cnh_number" name="cnh_number" value="<?php echo e(old('cnh_number', $driver->cnh_number)); ?>" placeholder="00000000000">
            </div>
            ...
            <div class="form-group">
                ...
                <select id="cnh_category" name="cnh_category" style="width: 100%; padding: 12px 15px; border-radius: 10px; border: 1px solid rgba(255, 255, 255, 0.2); background-color: var(--cor-principal); color: var(--cor-texto-claro); font-size: 1em; font-family: inherit;">
                    ...
                    <option value="A" <?php echo e(old('cnh_category', $driver->cnh_category) === 'A' ? 'selected' : ''); ?>>A - Motocicleta</option>
                    <option value="B" <?php echo e(old('cnh_category', $driver->cnh_category) === 'B' ? 'selected' : ''); ?>>B - Carro</option>
                    <option value="C" <?php echo e(old('cnh_category', $driver->cnh_category) === 'C' ? 'selected' : ''); ?>>C - Caminhão</option>
                    <option value="D" <?php echo e(old('cnh_category', $driver->cnh_category) === 'D' ? 'selected' : ''); ?>>D - Ônibus</option>
                    <option value="E" <?php echo e(old('cnh_category', $driver->cnh_category) === 'E' ? 'selected' : ''); ?>>E - Carreta</option>
                    <option value="AB" <?php echo e(old('cnh_category', $driver->cnh_category) === 'AB' ? 'selected' : ''); ?>>AB - Moto e Carro</option>
                    <option value="AC" <?php echo e(old('cnh_category', $driver->cnh_category) === 'AC' ? 'selected' : ''); ?>>AC - Moto e Caminhão</option>
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="date" id="cnh_expiry_date" name="cnh_expiry_date" value="<?php echo e(old('cnh_expiry_date', $driver->cnh_expiry_date ? $driver->cnh_expiry_date->format('Y-m-d') : '')); ?>">
            </div>
        </div>
        ...
        <div class="vehicle-section">
            ...
            <?php if($assignedVehicles && $assignedVehicles->count() > 0): ?>
                <div class="vehicle-list">
                    <?php $__currentLoopData = $assignedVehicles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $vehicle): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="vehicle-card">
                            <div class="vehicle-card-header">
                                <strong><?php echo e($vehicle->plate ?? '---'); ?></strong>
                                <span class="vehicle-subtitle"><?php echo e($vehicle->status_label); ?></span>
                            </div>
                            <p style="margin: 0; color: rgba(245, 245, 245, 0.7);">
                                <?php echo e($vehicle->brand ?? 'Marca não informada'); ?> - <?php echo e($vehicle->model ?? 'Modelo não informado'); ?>

                            </p>
                            <p class="info-text" style="margin: 5px 0;">
                                <?php echo e($vehicle->color ? "Cor: {$vehicle->color}" : 'Cor não informada'); ?>

                            </p>
                            <p class="info-text" style="margin: 0;">
                                Vinculado em: <?php echo e($vehicle->pivot->assigned_at ? \Carbon\Carbon::parse($vehicle->pivot->assigned_at)->format('d/m/Y') : 'Não informado'); ?>
        ...
        <div class="form-group">
            ...
            <!-- Photo Gallery -->
            <?php if($driver->photos && $driver->photos->count() > 0): ?>
            <div class="photo-gallery" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(100px, 1fr)); gap: 10px; margin-bottom: 15px;">
                <?php $__currentLoopData = $driver->photos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $photo): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="photo-gallery-item" style="position: relative; border-radius: 10px; overflow: hidden; aspect-ratio: 1; background: var(--cor-principal); border: 2px solid <?php echo e($photo->is_primary ? 'var(--cor-acento)' : 'rgba(255,255,255,0.2)'); ?>;">
                    <img src="<?php echo e($photo->url); ?>" alt="Foto" style="width: 100%; height: 100%; object-fit: cover;">
                    ...
                    <div style="position: absolute; bottom: 0; left: 0; right: 0; background: linear-gradient(to top, rgba(0,0,0,0.7), transparent); padding: 5px; display: flex; gap: 5px;">
                        <?php if(!$photo->is_primary): ?>
                        <button type="button" onclick="setPrimaryPhoto(<?php echo e($photo->id); ?>)" style="flex: 1; padding: 5px; background: rgba(255,107,53,0.8); color: white; border: none; border-radius: 5px; font-size: 0.7em; cursor: pointer;">
                            <i class="fas fa-star"></i>
                        </button>
                        <?php endif; ?>
                        <button type="button" onclick="deletePhoto(<?php echo e($photo->id); ?>)" style="flex: 1; padding: 5px; background: rgba(244,67,54,0.8); color: white; border: none; border-radius: 5px; font-size: 0.7em; cursor: pointer;">
...
<!-- Activity History -->
<?php if(isset($activityLog) && $activityLog->count() > 0): ?>
<div class="profile-card">
    <h3 style="color: var(--cor-acento); margin-bottom: 20px; font-size: 1.2em;">
    ...
    <div class="activity-list" style="display: flex; flex-direction: column; gap: 15px;">
        ...
        <div class="activity-item" style="background: rgba(255,255,255,0.05); padding: 15px; border-radius: 10px; border-left: 3px solid var(--cor-acento);">
            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 8px;">
                <div>
                    <strong style="color: var(--cor-texto-claro);"><?php echo e($activity->description); ?></strong>
            ...
            <?php if($activity->properties && count($activity->properties->get('attributes', [])) > 0): ?>
            <div style="margin-top: 10px; padding-top: 10px; border-top: 1px solid rgba(255,255,255,0.1);">
                <details style="color: rgba(245, 245, 245, 0.7); font-size: 0.85em;">
                    ...
                    <div style="margin-top: 10px; display: grid; gap: 5px;">
                        <?php $__currentLoopData = $activity->properties->get('attributes', []); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $value): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php if(isset($activity->properties->get('old', [])[$key]) && $activity->properties->get('old', [])[$key] != $value): ?>
                        <div style="padding: 5px; background: rgba(255,255,255,0.03); border-radius: 5px;">
                            <strong><?php echo e($key); ?>:</strong>
                            <span style="text-decoration: line-through; opacity: 0.6;"><?php echo e($activity->properties->get('old', [])[$key] ?? 'N/A'); ?></span>
                            <span style="color: var(--cor-acento);">→</span>
                            <span><?php echo e($value ?? 'N/A'); ?></span>
                        </div>
                        <?php endif; ?>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </div>
...
<?php echo $__env->make('driver.layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/resources/views/driver/profile.blade.php ENDPATH**/ ?>