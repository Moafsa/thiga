...
<?php $__env->startSection('title', 'Novo Veículo - TMS SaaS'); ?>
<?php $__env->startSection('page-title', 'Novo Veículo'); ?>
...
<form action="<?php echo e(route('vehicles.store')); ?>" method="POST" style="background-color: var(--cor-secundaria); padding: 30px; border-radius: 15px;">
    <?php echo csrf_field(); ?>
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px; margin-bottom: 30px;">
        <div>
            ...
            <input type="text" name="plate" value="<?php echo e(old('plate')); ?>" required 
        ...
        <div>
            ...
            <input type="text" name="renavam" value="<?php echo e(old('renavam')); ?>" 
                   style="width: 100%; padding: 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.2); background: var(--cor-principal); color: var(--cor-texto-claro);">
        </div>
        <div>
            <label style="color: var(--cor-texto-claro); display: block; margin-bottom: 8px;">Chassi</label>
            <input type="text" name="chassis" value="<?php echo e(old('chassis')); ?>" 
                   style="width: 100%; padding: 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.2); background: var(--cor-principal); color: var(--cor-texto-claro);">
        </div>
        <div>
        ...
        <div>
            ...
            <input type="number" name="year" value="<?php echo e(old('year')); ?>" min="1900" max="<?php echo e(date('Y') + 1); ?>"
                   style="width: 100%; padding: 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.2); background: var(--cor-principal); color: var(--cor-texto-claro);">
        </div>
        <div>
        ...
        <div>
            ...
            <select name="fuel_type" style="width: 100%; padding: 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.2); background: var(--cor-principal); color: var(--cor-texto-claro);">
                ...
                <?php $__currentLoopData = $fuelTypes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $fuel): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($fuel); ?>" <?php echo e(old('fuel_type') === $fuel ? 'selected' : ''); ?>><?php echo e($fuel); ?></option>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </select>
        </div>
    </div>
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px; margin-bottom: 30px;">
        <div>
            ...
            <input type="number" name="capacity_weight" value="<?php echo e(old('capacity_weight')); ?>" step="0.01" min="0"
                   style="width: 100%; padding: 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.2); background: var(--cor-principal); color: var(--cor-texto-claro);">
        </div>
        <div>
            <label style="color: var(--cor-texto-claro); display: block; margin-bottom: 8px;">Capacidade de Volume (m³)</label>
            <input type="number" name="capacity_volume" value="<?php echo e(old('capacity_volume')); ?>" step="0.01" min="0"
                   style="width: 100%; padding: 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.2); background: var(--cor-principal); color: var(--cor-texto-claro);">
        </div>
        <div>
            <label style="color: var(--cor-texto-claro); display: block; margin-bottom: 8px;">Eixos</label>
            <input type="number" name="axles" value="<?php echo e(old('axles')); ?>" min="1" max="10"
                   style="width: 100%; padding: 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.2); background: var(--cor-principal); color: var(--cor-texto-claro);">
        </div>
        <div>
            <label style="color: var(--cor-texto-claro); display: block; margin-bottom: 8px;">Odômetro Atual (km)</label>
            <input type="number" name="current_odometer" value="<?php echo e(old('current_odometer', 0)); ?>" min="0"
                   style="width: 100%; padding: 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.2); background: var(--cor-principal); color: var(--cor-texto-claro);">
        </div>
    </div>
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px; margin-bottom: 30px;">
        <div>
            ...
            <input type="number" name="maintenance_interval_km" value="<?php echo e(old('maintenance_interval_km', 10000)); ?>" min="0"
                   style="width: 100%; padding: 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.2); background: var(--cor-principal); color: var(--cor-texto-claro);">
        </div>
        <div>
            <label style="color: var(--cor-texto-claro); display: block; margin-bottom: 8px;">Intervalo de Manutenção (dias)</label>
            <input type="number" name="maintenance_interval_days" value="<?php echo e(old('maintenance_interval_days', 90)); ?>" min="0"
                   style="width: 100%; padding: 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.2); background: var(--cor-principal); color: var(--cor-texto-claro);">
        </div>
    </div>
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px; margin-bottom: 30px;">
        <div>
            ...
            <input type="date" name="insurance_expiry_date" value="<?php echo e(old('insurance_expiry_date')); ?>"
                   style="width: 100%; padding: 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.2); background: var(--cor-principal); color: var(--cor-texto-claro);">
        </div>
        <div>
            <label style="color: var(--cor-texto-claro); display: block; margin-bottom: 8px;">Data de Vencimento da Vistoria</label>
            <input type="date" name="inspection_expiry_date" value="<?php echo e(old('inspection_expiry_date')); ?>"
                   style="width: 100%; padding: 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.2); background: var(--cor-principal); color: var(--cor-texto-claro);">
        </div>
        <div>
            <label style="color: var(--cor-texto-claro); display: block; margin-bottom: 8px;">Data de Vencimento do Licenciamento</label>
            <input type="date" name="registration_expiry_date" value="<?php echo e(old('registration_expiry_date')); ?>"
                   style="width: 100%; padding: 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.2); background: var(--cor-principal); color: var(--cor-texto-claro);">
        </div>
    </div>
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px; margin-bottom: 30px;">
        <div>
            ...
            <select name="status" style="width: 100%; padding: 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.2); background: var(--cor-principal); color: var(--cor-texto-claro);">
                <option value="available" <?php echo e(old('status', 'available') === 'available' ? 'selected' : ''); ?>>Disponível</option>
                <option value="in_use" <?php echo e(old('status') === 'in_use' ? 'selected' : ''); ?>>Em Uso</option>
                <option value="maintenance" <?php echo e(old('status') === 'maintenance' ? 'selected' : ''); ?>>Em Manutenção</option>
                <option value="inactive" <?php echo e(old('status') === 'inactive' ? 'selected' : ''); ?>>Inativo</option>
            </select>
        </div>
        <div>
            ...
            <select name="ownership_type" required style="width: 100%; padding: 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.2); background: var(--cor-principal); color: var(--cor-texto-claro);">
                <option value="fleet" <?php echo e(old('ownership_type', 'fleet') === 'fleet' ? 'selected' : ''); ?>>Frota (pode ter manutenções e despesas)</option>
                <option value="third_party" <?php echo e(old('ownership_type') === 'third_party' ? 'selected' : ''); ?>>Terceiro (não pode ter manutenções nem despesas)</option>
            </select>
        ...
        <div style="display: flex; align-items: center; margin-top: 30px;">
            <label style="color: var(--cor-texto-claro); display: flex; align-items: center; gap: 10px; cursor: pointer;">
                <input type="checkbox" name="is_active" value="1" <?php echo e(old('is_active', true) ? 'checked' : ''); ?> style="width: 20px; height: 20px;">
                Ativo
            </label>
        </div>
    </div>
    ...
    <div>
        ...
        <textarea name="notes" rows="4" style="width: 100%; padding: 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.2); background: var(--cor-principal); color: var(--cor-texto-claro);"><?php echo e(old('notes')); ?></textarea>
    </div>

    <div style="display: flex; gap: 15px; justify-content: flex-end; margin-top: 30px;">
        <a href="<?php echo e(route('vehicles.index')); ?>" class="btn-secondary">Cancelar</a>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/resources/views/vehicles/create.blade.php ENDPATH**/ ?>