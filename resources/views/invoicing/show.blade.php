...
<head>
    ...
    <title>Fatura #{{ $invoice->invoice_number }} - TMS SaaS</title>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center py-4">
                ...
                <div class="flex items-center space-x-4">
                    <a href="{{ route('invoicing.index') }}" class="text-gray-700 hover:text-gray-900">
        ...
        <div class="bg-white rounded-lg shadow-lg p-8">
            ...
            <div class="border-b-2 border-gray-200 pb-6 mb-6">
                <div class="flex justify-between items-start">
                    <div>
                        ...
                        <p class="text-gray-600">Número: <span class="font-semibold">{{ $invoice->invoice_number }}</span></p>
                        <p class="text-gray-600">Data de Emissão: {{ $invoice->issue_date->format('d/m/Y') }}</p>
                        <p class="text-gray-600">Data de Vencimento: {{ $invoice->due_date->format('d/m/Y') }}</p>
                    </div>
            ...
            <div class="mb-6">
                ...
                <div class="bg-gray-50 rounded-lg p-4">
                    ...
                    <p class="text-gray-600">{{ $invoice->client->cnpj }}</p>
                    <p class="text-gray-600">{{ $invoice->client->address }}</p>
                    <p class="text-gray-600">{{ $invoice->client->city }}/{{ $invoice->client->state }} - {{ $invoice->client->zip_code }}</p>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($invoice->items as $item)
                                <tr>
                                    ...
                                    <td class="px-4 py-3 text-sm text-gray-600 text-right">R$ {{ number_format($item->unit_price, 2, ',', '.') }}</td>
                                    <td class="px-4 py-3 text-sm font-semibold text-gray-900 text-right">R$ {{ number_format($item->total_price, 2, ',', '.') }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                    ...
                    <div class="space-y-2">
                        ...
                        @if($invoice->isPaid())
                            <div class="flex justify-between text-green-600 pt-2">
                                ...
                                <span>R$ {{ number_format($invoice->total_paid, 2, ',', '.') }}</span>
                            </div>
                            @if($invoice->remaining_balance > 0)
                                <div class="flex justify-between text-orange-600 pt-2">
                                    <span>Saldo Restante:</span>
                                    <span>R$ {{ number_format($invoice->remaining_balance, 2, ',', '.') }}</span>
                                </div>
                            @endif
                        @endif
                    </div>
                        ...
                        @foreach($invoice->payments as $payment)
                            <div class="bg-gray-50 rounded-lg p-4 flex justify-between items-center">
                                <div>
                                    ...
                                    <p class="text-sm text-gray-600">{{ $payment->paid_at ? $payment->paid_at->format('d/m/Y') : $payment->due_date->format('d/m/Y') }}</p>
