...
@section('content')
    <div style="width: 100%;">
        ...
        <div class="page-header"
            style="flex-shrink: 0; display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px;">
            <div style="display: flex; align-items: center; gap: 20px;">
                ...
                <!-- Mode Switcher (Links) -->
                <div
                    style="background: var(--cor-secundaria); padding: 5px; border-radius: 12px; display: flex; gap: 5px; border: 1px solid rgba(255,255,255,0.1);">
                    <a href="{{ route('routes.create-smart') }}"
                        style="background: transparent; color: var(--cor-texto-claro); opacity: 0.7; padding: 8px 16px; border-radius: 8px; text-decoration: none; display: flex; align-items: center; font-family: inherit; font-size: 0.9em; transition: all 0.2s;">
                        <i class="fas fa-map-marked-alt" style="margin-right: 8px;"></i> Smart
                    </a>
                    <a href="{{ route('routes.create') }}"
                        style="background: var(--cor-acento); color: #000; font-weight: 600; padding: 8px 16px; border-radius: 8px; text-decoration: none; display: flex; align-items: center; font-family: inherit; font-size: 0.9em;">
        ...
        <div style="padding-top: 10px;">
            <form action="{{ route('routes.store') }}" method="POST" enctype="multipart/form-data"
                style="background-color: var(--cor-secundaria); padding: 30px; border-radius: 15px;">
                @csrf
                <div
                        ...
                        <select name="driver_id" id="driver_id"
                            ...
                            @foreach($drivers as $driver)
                                <option value="{{ $driver->id }}"
                                    data-vehicles="{{ $driver->vehicles->pluck('id')->toJson() }}">
                                    {{ $driver->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="vehicle_id" id="vehicle_id"
                            ...
                            @foreach($vehicles as $vehicle)
                                <option value="{{ $vehicle->id }}" data-driver-vehicles>{{ $vehicle->formatted_plate }}
                                    @if($vehicle->brand && $vehicle->model) - {{ $vehicle->brand }} {{ $vehicle->model }} @endif
                                </option>
                            @endforeach
                        </select>
                        <small style="color: rgba(245, 245, 245, 0.6);">Apenas veículos atribuídos ao motorista selecionado
                            serão
                            exibidos</small>
                    </div>
                    <div>
                        <label style="color: var(--cor-texto-claro); display: block; margin-bottom: 8px;">Data
                            Agendada</label>
                ...
                <div
                    style="margin-bottom: 20px; background-color: var(--cor-principal); padding: 20px; border-radius: 10px;">
                    ...
                    <div style="margin-bottom: 15px;">
                        <label
                            style="display: flex; align-items: center; padding: 10px; background: var(--cor-secundaria); border-radius: 5px; margin-bottom: 10px; cursor: pointer;">
                            <input type="radio" name="start_address_type" value="branch" id="start_type_branch" {{ old('start_address_type', 'branch') == 'branch' ? 'checked' : '' }}
                                style="margin-right: 10px;">
                            <span style="color: var(--cor-texto-claro);">Depósito/Filial da Empresa</span>
                        </label>

                        <div id="branch_selection"
                            style="margin-left: 30px; margin-bottom: 15px; {{ old('start_address_type', 'branch') != 'branch' ? 'display: none;' : '' }}">
                            <div style="display: flex; gap: 10px; align-items: flex-start;">
                                <select name="branch_id" id="branch_id"
                                    style="flex: 1; padding: 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.2); background: var(--cor-secundaria); color: var(--cor-texto-claro);">
                                ...
                                <button type="button" id="add-branch-btn" class="btn-secondary"
                                    style="padding: 12px 16px; white-space: nowrap;" title="Adicionar novo Depósito/Filial">
                        ...
                        <label
                            style="display: flex; align-items: center; padding: 10px; background: var(--cor-secundaria); border-radius: 5px; margin-bottom: 10px; cursor: pointer;">
                            <input type="radio" name="start_address_type" value="current_location" id="start_type_current"
                                {{ old('start_address_type') == 'current_location' ? 'checked' : '' }}
                                style="margin-right: 10px;">
                            <span style="color: var(--cor-texto-claro);">Localização Atual do Motorista</span>
                        </label>
                        <small
                            style="color: rgba(245, 245, 245, 0.6); display: block; margin-left: 30px; margin-bottom: 15px;">Será
                            usada a localização atual do motorista selecionado</small>
                        ...
                        <div id="manual_address"
                            style="margin-left: 30px; display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; {{ old('start_address_type') != 'manual' ? 'display: none;' : '' }}">
                            <div>
                                <label
                                    style="color: var(--cor-texto-claro); display: block; margin-bottom: 5px; font-size: 0.9em;">Endereço
                                    *</label>
                                <input type="text" name="start_address" value="{{ old('start_address') }}"
                            ...
                            <div>
                                <label
                                    style="color: var(--cor-texto-claro); display: block; margin-bottom: 5px; font-size: 0.9em;">Cidade
                                    *</label>
                                <input type="text" name="start_city" value="{{ old('start_city') }}" placeholder="Cidade"
                                    style="width: 100%; padding: 10px; border-radius: 5px; border: 1px solid rgba(255,255,255,0.2); background: var(--cor-secundaria); color: var(--cor-texto-claro);">
                            </div>
                            <div>
                                <label
                                    style="color: var(--cor-texto-claro); display: block; margin-bottom: 5px; font-size: 0.9em;">Estado
                                    *</label>
                                <input type="text" name="start_state" value="{{ old('start_state') }}" placeholder="UF"
                                    maxlength="2"
                                    style="width: 100%; padding: 10px; border-radius: 5px; border: 1px solid rgba(255,255,255,0.2); background: var(--cor-secundaria); color: var(--cor-texto-claro); text-transform: uppercase;">
                            </div>
                            <div>
                                <label
                                    style="color: var(--cor-texto-claro); display: block; margin-bottom: 5px; font-size: 0.9em;">CEP</label>
                                <input type="text" name="start_zip_code" value="{{ old('start_zip_code') }}"
                ...
                <div
                    ...
                    <p style="color: rgba(245, 245, 245, 0.7); font-size: 0.9em; margin-bottom: 15px;">
                        Adicione os endereços da rota. O sistema criará cargas automaticamente conectando os endereços
                        sequencialmente.
                    </p>
                ...
                <div
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                        ...
                        <a href="{{ route('cte-xmls.index') }}" class="btn-secondary"
                    ...
                    <button type="button" id="add-cte-xml-number-btn" class="btn-secondary"
                ...
                <div
                    ...
                    <div
                        style="max-height: 300px; overflow-y: auto; border: 1px solid rgba(255,255,255,0.2); border-radius: 8px; padding: 15px;">
                        @forelse($availableShipments as $shipment)
                            <label
                                style="display: flex; align-items: center; padding: 10px; margin-bottom: 5px; background: var(--cor-secundaria); border-radius: 5px;">
                                <input type="checkbox" name="shipment_ids[]" value="{{ $shipment->id }}"
                                    style="margin-right: 10px;">
                                <span style="color: var(--cor-texto-claro);">{{ $shipment->tracking_number }} -
                                    {{ $shipment->title }}</span>
                            </label>
                        @empty
                ...
                @if(isset($availableCargo) && $availableCargo->count() > 0)
                    <div
                        style="margin-bottom: 20px; background-color: var(--cor-principal); padding: 20px; border-radius: 10px;">
                        <h3 style="color: var(--cor-acento); margin-bottom: 15px;">Cargas Disponíveis para Coleta (de Propostas)
                        </h3>
                        <div
                            style="max-height: 300px; overflow-y: auto; border: 1px solid rgba(255,255,255,0.2); border-radius: 8px; padding: 15px;">
                            @foreach($availableCargo as $cargo)
                                ...
                                <label
                                    style="display: flex; align-items: center; padding: 10px; margin-bottom: 5px; background: var(--cor-secundaria); border-radius: 5px;">
                                    <input type="checkbox" name="available_cargo_ids[]" value="{{ $cargo->id }}"
                                        style="margin-right: 10px;">
                                    <div style="flex: 1; color: var(--cor-texto-claro);">
                                        <div style="font-weight: 600;">{{ $proposal->proposal_number }} - {{ $proposal->title }}
                                        </div>
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <textarea name="description" rows="3"
                        style="width: 100%; padding: 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.2); background: var(--cor-principal); color: var(--cor-texto-claro);">{{ old('description') }}</textarea>
                </div>
                ...
                <div
                    ...
                    <p style="color: rgba(245, 245, 245, 0.7); font-size: 0.9em; margin-bottom: 15px;">
                        Configure a taxa mínima de frete para esta rota. Esta taxa terá prioridade sobre a taxa mínima da
                        tabela de
                        frete.
                    </p>
                    ...
                    <div
                        style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px; margin-bottom: 20px;">
                        <div>
                            <label style="color: var(--cor-texto-claro); display: block; margin-bottom: 8px;">Tipo de Taxa
                                Mínima</label>
                            <select name="min_freight_rate_type" id="min_freight_rate_type"
                                style="width: 100%; padding: 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.2); background: var(--cor-secundaria); color: var(--cor-texto-claro);">
                                <option value="">Nenhuma (usar da tabela)</option>
                                <option value="percentage" {{ old('min_freight_rate_type') === 'percentage' ? 'selected' : '' }}>
                                    Percentual sobre NF</option>
                                <option value="fixed" {{ old('min_freight_rate_type') === 'fixed' ? 'selected' : '' }}>Valor
                                    Fixo (R$)
                                </option>
                            </select>
                        </div>
                        <div>
                            <label style="color: var(--cor-texto-claro); display: block; margin-bottom: 8px;"
                                id="min_freight_rate_value_label">
                                Valor da Taxa Mínima
                            </label>
                            <input type="number" name="min_freight_rate_value" id="min_freight_rate_value"
                                value="{{ old('min_freight_rate_value') }}" step="0.01" min="0"
                                style="width: 100%; padding: 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.2); background: var(--cor-secundaria); color: var(--cor-texto-claro);"
                                placeholder="0.00">
                            <small style="color: rgba(245, 245, 245, 0.6); display: block; margin-top: 5px;"
                                id="min_freight_rate_value_help">
                    ...
                    <div id="min_freight_rate_days_section" style="display: none;">
                        <label style="color: var(--cor-texto-claro); display: block; margin-bottom: 8px;">Dias da Semana
                            para
                            Aplicar Taxa Mínima</label>
                        <p style="color: rgba(245, 245, 245, 0.7); font-size: 0.9em; margin-bottom: 10px;">
                            Selecione os dias da semana em que esta taxa mínima será aplicada. Se nenhum dia for
                            selecionado, aplica
                            em todos os dias.
                        </p>
                        <div style="display: flex; flex-wrap: wrap; gap: 10px;">
                            ...
                            @foreach($daysOfWeek as $dayNumber => $dayName)
                                <label
                                    style="display: flex; align-items: center; padding: 8px 15px; background: var(--cor-secundaria); border-radius: 5px; cursor: pointer;">
                ...
                <div style="display: flex; gap: 15px; justify-content: flex-end;">
                    ...
                    <button type="submit" formaction="{{ route('fast-routes.store') }}" class="btn-primary"
                        style="background-color: #4CAF50; border-color: #4CAF50;"
                        title="Cria a rota automaticamente com as cargas selecionadas">
                        <i class="fas fa-bolt"></i> Criar Rota Rápida
                    </button>
            ...
            @push('scripts')
                <script>
                    document.addEventListener('DOMContentLoaded', function () {
                        ...
                        const oldAddresses = []; // DEBUG: @json(old('addresses', []), 15);
