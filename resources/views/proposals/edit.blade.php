...
@section('title', 'Editar Proposta Comercial - TMS SaaS')
@section('page-title', 'Editar Proposta Comercial')
...
<div class="proposal-container" id="proposal-content">
    <header class="proposal-header">
        <div>
            ...
            <p style="margin: 5px 0 0; color: #555;">Proposta Nº: {{ $proposal->proposal_number }} | Data: {{ $proposal->created_at->format('d/m/Y') }}</p>
        </div>
    </header>
    ...
    <main class="proposal-content">
        ...
        <section class="calculator-section">
            ...
            <form id="freight-calculator-form" action="{{ route('proposals.update', $proposal) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="form-grid">
                        ...
                        <select name="salesperson_id" id="salesperson_id" required>
                            ...
                            @foreach($salespeople as $salesperson)
                                <option value="{{ $salesperson->id }}" 
                                        ...
                                        {{ (old('salesperson_id', $proposal->salesperson_id) == $salesperson->id) ? 'selected' : '' }}>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" id="weight" name="weight" step="0.01" min="0" placeholder="Ex: 55" value="{{ old('weight', $proposal->weight) }}" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" id="cubage" name="cubage" step="0.01" min="0" placeholder="Ex: 0.5" value="{{ old('cubage', $proposal->cubage) }}">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" id="discount_percentage" name="discount_percentage" step="0.01" min="0" max="100" value="{{ old('discount_percentage', $proposal->discount_percentage) }}" placeholder="0.00">
                    </div>

                    <div class="form-group">
                        <label for="valid_until">Válido até</label>
                        <input type="date" id="valid_until" name="valid_until" min="{{ date('Y-m-d', strtotime('+1 day')) }}" value="{{ old('valid_until', $proposal->valid_until ? $proposal->valid_until->format('Y-m-d') : '') }}">
                    </div>
                </div>
                ...
                <div class="form-grid" style="margin-top: 20px;">
                    <div class="form-group full-width">
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $proposal->title) }}" required placeholder="Ex: Proposta de Frete - Belo Horizonte">
                        @error('title')
                            <span class="error-message">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group full-width">
                        ...
                        <textarea name="description" id="description" rows="3" placeholder="Descrição da proposta...">{{ old('description', $proposal->description) }}</textarea>
                        @error('description')
                            <span class="error-message">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group full-width">
                        ...
                        <textarea name="notes" id="notes" rows="3" placeholder="Observações adicionais...">{{ old('notes', $proposal->notes) }}</textarea>
                    </div>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        let calculatedFreightValue = {{ old('base_value', $proposal->base_value) }};
        ...
        freightForm.addEventListener('submit', function(e) {
            ...
            if (calculatedFreightValue > 0) {
                ...
                if (!document.getElementById('base_value_hidden')) {
                    ...
                    hiddenInput.id = 'base_value_hidden';
                    hiddenInput.name = 'base_value';
                    hiddenInput.value = {{ $proposal->base_value ?? 0 }};
                    freightForm.appendChild(hiddenInput);
                }
            }
