...
<div class="page-header">
    ...
    <div style="display: flex; gap: 10px; flex-wrap: wrap;">
        <a href="{{ route('freight-table-categories.index') }}" class="btn-secondary" style="background-color: #9c27b0; border-color: #9c27b0;">
...
@else
    ...
    @foreach($categories as $category)
        ...
        @endphp
        @if($categoryTables->count() > 0)
            <div class="category-accordion" style="border-left-color: {{ $category->color ?? '#FF6B35' }};">
                <div class="category-header" onclick="toggleCategory({{ $category->id }})">
                    <div class="category-title">
                        <div class="category-color" style="background-color: {{ $category->color ?? '#FF6B35' }};"></div>
                        <div>
                            <h3 class="category-name">
                                {{ $category->name }}
                                <span class="category-count">({{ $categoryTables->count() }} {{ $categoryTables->count() == 1 ? 'tabela' : 'tabelas' }})</span>
                            </h3>
                        </div>
                    </div>
                        ...
                        @foreach($categoryTables as $table)
                            <div class="table-card">
                                    ...
                                    <div class="table-card-title">
                                        ...
                                        <div>
                                            ...
                                            @if($table->description)
                                                <div style="opacity: 0.7; font-size: 0.85em; margin-top: 5px;">{{ Str::limit($table->description, 60) }}</div>
                                            @endif
                                        </div>
                                    </div>
                                ...
                                <div class="table-card-info">
                                    <div>
                                        <strong>Destino:</strong> {{ $table->destination_name }}
                                        @if($table->destination_state)
                                            - {{ $table->destination_state }}
                                        @endif
                                    </div>
                                    @if($table->client)
                                        <div><strong>Cliente:</strong> {{ $table->client->name }}</div>
                                    @endif
                                    <div>
                                        <strong>Tipo:</strong> 
                                        <span class="status-badge" style="background-color: {{ $table->destination_type === 'city' ? 'rgba(33, 150, 243, 0.2)' : ($table->destination_type === 'region' ? 'rgba(156, 39, 176, 0.2)' : 'rgba(255, 152, 0, 0.2)') }}; color: {{ $table->destination_type === 'city' ? '#2196f3' : ($table->destination_type === 'region' ? '#9c27b0' : '#ff9800') }}; padding: 2px 8px; border-radius: 4px; font-size: 0.85em;">
                                            {{ ucfirst(str_replace('_', ' ', $table->destination_type)) }}
                                        </span>
                                    </div>
                                    <div>
                                        <strong>Status:</strong> 
                                        <span class="status-badge" style="background-color: {{ $table->is_active ? 'rgba(76, 175, 80, 0.2)' : 'rgba(244, 67, 54, 0.2)' }}; color: {{ $table->is_active ? '#4caf50' : '#f44336' }}; padding: 2px 8px; border-radius: 4px; font-size: 0.85em;">
                                            {{ $table->is_active ? 'Ativa' : 'Inativa' }}
                                        </span>
                                        @if($table->visible_to_clients)
    ...
    {{-- Tabelas sem categoria --}}
    @if($uncategorizedTables->count() > 0)
        <div class="uncategorized-section">
            <div class="uncategorized-title">
                <i class="fas fa-folder-open"></i>
                Sem Categoria ({{ $uncategorizedTables->count() }} {{ $uncategorizedTables->count() == 1 ? 'tabela' : 'tabelas' }})
            </div>
            <div>
