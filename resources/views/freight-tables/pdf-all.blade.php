...
<body>
    ...
    <div class="company-info">
        <strong>{{ $tenant->name ?? 'Transportadora' }}</strong>
        @if($tenant->email || $tenant->phone)
            <br>
            @if($tenant->email)Email: {{ $tenant->email }}@endif
            @if($tenant->email && $tenant->phone) | @endif
            @if($tenant->phone)Telefone: {{ $tenant->phone }}@endif
        @endif
    </div>
    @endif
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                <td class="destination-name">{{ $table->destination_name }}@if($table->is_default)*@endif</td>
                <td>R$ {{ number_format($table->weight_0_30 ?? 0, 2, ',', '.') }}</td>
                <td>R$ {{ number_format($table->weight_31_50 ?? 0, 2, ',', '.') }}</td>
                <td>R$ {{ number_format($table->weight_51_70 ?? 0, 2, ',', '.') }}</td>
                <td>R$ {{ number_format($table->weight_71_100 ?? 0, 2, ',', '.') }}</td>
                <td>R$ {{ number_format($table->weight_over_100_rate ?? 0, 4, ',', '.') }}</td>
                <td>R$ {{ number_format($table->ctrc_tax ?? 0, 2, ',', '.') }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
    ...
    <div class="footer">
        ...
        <p>Total de {{ $freightTables->count() }} destino(s) cadastrado(s)</p>
    </div>
</body>
</html>
