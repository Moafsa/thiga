...
<head>
    ...
    <title>Tabela de Frete - {{ $freightTable->name }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <div class="company-info">
        <h3>{{ $tenant->name ?? 'Transportadora' }}</h3>
        @if($tenant->email)
    ...
    <div class="info-section">
        ...
        <div class="info-grid">
            <div class="info-row">
                ...
                <div class="info-value">{{ $freightTable->name }}</div>
            </div>
            @if($freightTable->description)
            <div class="info-row">
                <div class="info-label">Descrição:</div>
                <div class="info-value">{{ $freightTable->description }}</div>
            </div>
            @endif
            <div class="info-row">
                <div class="info-label">Destino:</div>
                <div class="info-value">{{ $freightTable->destination_name }}</div>
            </div>
            @if($freightTable->destination_state)
            <div class="info-row">
                <div class="info-label">Estado:</div>
                <div class="info-value">{{ $freightTable->destination_state }}</div>
            </div>
            @endif
            <div class="info-row">
                <div class="info-label">Tipo:</div>
                <div class="info-value">{{ ucfirst(str_replace('_', ' ', $freightTable->destination_type)) }}</div>
            </div>
            @if($freightTable->cep_range_start && $freightTable->cep_range_end)
            <div class="info-row">
                <div class="info-label">Faixa de CEP:</div>
                <div class="info-value">{{ $freightTable->cep_range_start }} - {{ $freightTable->cep_range_end }}</div>
            </div>
            @endif
            <div class="info-row">
    ...
    <div class="info-section">
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    <td>0 a 30 kg</td>
                    <td class="text-right text-bold">R$ {{ number_format($freightTable->weight_0_30, 2, ',', '.') }}</td>
                </tr>
                @endif
                @if($freightTable->weight_31_50)
                <tr>
                    <td>31 a 50 kg</td>
                    <td class="text-right text-bold">R$ {{ number_format($freightTable->weight_31_50, 2, ',', '.') }}</td>
                </tr>
                @endif
                @if($freightTable->weight_51_70)
                <tr>
                    <td>51 a 70 kg</td>
                    <td class="text-right text-bold">R$ {{ number_format($freightTable->weight_51_70, 2, ',', '.') }}</td>
                </tr>
                @endif
                @if($freightTable->weight_71_100)
                <tr>
                    <td>71 a 100 kg</td>
                    <td class="text-right text-bold">R$ {{ number_format($freightTable->weight_71_100, 2, ',', '.') }}</td>
                </tr>
                @endif
                @if($freightTable->weight_over_100_rate)
                <tr>
                    <td>Acima de 100 kg (por kg)</td>
                    <td class="text-right text-bold">R$ {{ number_format($freightTable->weight_over_100_rate, 4, ',', '.') }}</td>
                </tr>
                @endif
            </tbody>
        </table>
    ...
    <div class="info-section">
        ...
        <div class="info-grid">
            ...
            <div class="info-row">
                ...
                <div class="info-value text-bold">R$ {{ number_format($freightTable->ctrc_tax, 2, ',', '.') }}</div>
            </div>
            @endif
            @if($freightTable->ad_valorem_rate)
            <div class="info-row">
                <div class="info-label">Ad Valorem:</div>
                <div class="info-value text-bold">{{ number_format($freightTable->ad_valorem_rate * 100, 2, ',', '.') }}%</div>
            </div>
            @endif
            @if($freightTable->gris_rate)
            <div class="info-row">
                <div class="info-label">GRIS:</div>
                <div class="info-value text-bold">{{ number_format($freightTable->gris_rate * 100, 2, ',', '.') }}%</div>
            </div>
            @endif
            @if($freightTable->gris_minimum)
            <div class="info-row">
                <div class="info-label">GRIS Mínimo:</div>
                <div class="info-value text-bold">R$ {{ number_format($freightTable->gris_minimum, 2, ',', '.') }}</div>
            </div>
            @endif
            @if($freightTable->toll_per_100kg)
            <div class="info-row">
                <div class="info-label">Pedágio (por 100kg):</div>
                <div class="info-value text-bold">R$ {{ number_format($freightTable->toll_per_100kg, 2, ',', '.') }}</div>
            </div>
            @endif
            @if($freightTable->cubage_factor)
            <div class="info-row">
                <div class="info-label">Fator de Cubagem:</div>
                <div class="info-value text-bold">{{ number_format($freightTable->cubage_factor, 0, ',', '.') }} kg/m³</div>
            </div>
            @endif
            @if($freightTable->min_freight_rate_vs_nf)
            <div class="info-row">
                <div class="info-label">Frete Mínimo vs NF:</div>
                <div class="info-value text-bold">{{ number_format($freightTable->min_freight_rate_vs_nf * 100, 2, ',', '.') }}%</div>
            </div>
            @endif
        </div>
    </div>
    ...
    <div class="info-section">
        ...
        <div class="info-grid">
            ...
            <div class="info-row">
                ...
                <div class="info-value text-bold">R$ {{ number_format($freightTable->tde_markets, 2, ',', '.') }}</div>
            </div>
            @endif
            @if($freightTable->tde_supermarkets_cd)
            <div class="info-row">
                <div class="info-label">TDE Supermercados CD:</div>
                <div class="info-value text-bold">R$ {{ number_format($freightTable->tde_supermarkets_cd, 2, ',', '.') }}</div>
            </div>
            @endif
            @if($freightTable->palletization)
            <div class="info-row">
                <div class="info-label">Paletização:</div>
                <div class="info-value text-bold">R$ {{ number_format($freightTable->palletization, 2, ',', '.') }}</div>
            </div>
            @endif
            @if($freightTable->unloading_tax)
            <div class="info-row">
                <div class="info-label">Taxa de Descarga:</div>
                <div class="info-value text-bold">R$ {{ number_format($freightTable->unloading_tax, 2, ',', '.') }}</div>
            </div>
            @endif
        </div>
    </div>
    ...
    <div class="info-section">
        ...
        <div class="info-grid">
            ...
            <div class="info-row">
                ...
                <div class="info-value text-bold">+{{ number_format($freightTable->weekend_holiday_rate * 100, 0, ',', '.') }}%</div>
            </div>
            @endif
            @if($freightTable->redelivery_rate)
            <div class="info-row">
                <div class="info-label">Reentrega:</div>
                <div class="info-value text-bold">+{{ number_format($freightTable->redelivery_rate * 100, 0, ',', '.') }}%</div>
            </div>
            @endif
            @if($freightTable->return_rate)
            <div class="info-row">
                <div class="info-label">Devolução:</div>
                <div class="info-value text-bold">+{{ number_format($freightTable->return_rate * 100, 0, ',', '.') }}%</div>
            </div>
            @endif
        </div>
    </div>
