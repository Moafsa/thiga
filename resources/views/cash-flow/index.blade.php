...
@section('title', 'Fluxo de Caixa - TMS SaaS')
...
<div class="stats-grid">
    <div class="stat-card">
        ...
        <div class="stat-content">
            <h3 style="color: #4caf50;">R$ {{ number_format($stats['total_receivables'], 2, ',', '.') }}</h3>
            <p>Total Recebido</p>
            <p style="font-size: 0.85em; opacity: 0.7;">{{ $stats['receivables_count'] }} transações</p>
        </div>
    </div>
    <div class="stat-card">
        ...
        <div class="stat-content">
            <h3 style="color: #f44336;">R$ {{ number_format($stats['total_payables'], 2, ',', '.') }}</h3>
            <p>Total Pago</p>
            <p style="font-size: 0.85em; opacity: 0.7;">{{ $stats['payables_count'] }} transações</p>
        </div>
    </div>
    <div class="stat-card">
        <div class="stat-icon" style="background-color: {{ $stats['balance'] >= 0 ? 'rgba(76, 175, 80, 0.2)' : 'rgba(244, 67, 54, 0.2)' }}; color: {{ $stats['balance'] >= 0 ? '#4caf50' : '#f44336' }};">
            <i class="fas fa-balance-scale"></i>
        </div>
        <div class="stat-content">
            <h3 style="color: {{ $stats['balance'] >= 0 ? '#4caf50' : '#f44336' }};">R$ {{ number_format($stats['balance'], 2, ',', '.') }}</h3>
            <p>Saldo Líquido</p>
        </div>
    </div>
    <div class="stat-card">
        ...
        <div class="stat-content">
            <h3 style="color: #2196f3;">{{ $stats['transactions_count'] }}</h3>
    ...
    <div class="stat-card">
        ...
        <div class="stat-content">
            <h3 style="color: {{ $stats['final_balance'] >= 0 ? '#4caf50' : '#f44336' }};">R$ {{ number_format($stats['final_balance'], 2, ',', '.') }}</h3>
            <p>Saldo Final</p>
        </div>
    </div>
</div>
            ...
            <tbody>
                @if($stats['initial_balance'] != 0)
                    <tr style="background-color: rgba(255, 255, 255, 0.05); font-weight: 600;">
                        ...
                        <td style="text-align: right; font-weight: 600; color: {{ $stats['initial_balance'] >= 0 ? '#4caf50' : '#f44336' }};">
                            R$ {{ number_format($stats['initial_balance'], 2, ',', '.') }}
                        </td>
                    </tr>
                @endif
                ...
                @forelse($transactionsWithBalance as $transaction)
                    <tr>
                        ...
                        <td>
                            <span class="status-badge" style="background-color: {{ $transaction['type'] === 'receivable' ? 'rgba(76, 175, 80, 0.2)' : 'rgba(244, 67, 54, 0.2)' }}; color: {{ $transaction['type'] === 'receivable' ? '#4caf50' : '#f44336' }};">
                                {{ $transaction['type'] === 'receivable' ? 'Recebimento' : 'Pagamento' }}
                            </span>
                        </td>
                        ...
                        <td style="opacity: 0.8; font-size: 0.9em;">
                            ...
                            @endif
                            @if(isset($transaction['payment_method']) && $transaction['payment_method'])
                                <div style="font-size: 0.85em; margin-top: 3px;">{{ $transaction['payment_method'] }}</div>
                            @endif
                        </td>
                        ...
                        <td style="text-align: right; font-weight: 600; color: {{ $transaction['balance'] >= 0 ? '#4caf50' : '#f44336' }};">
                            R$ {{ number_format($transaction['balance'], 2, ',', '.') }}
                        </td>
                    </tr>
                @empty
                    <tr>
                ...
                @if($stats['final_balance'] != 0)
                    <tr style="background-color: rgba(255, 255, 255, 0.05); font-weight: 600;">
                        ...
                        <td style="text-align: right; font-weight: 600; color: {{ $stats['final_balance'] >= 0 ? '#4caf50' : '#f44336' }};">
                            R$ {{ number_format($stats['final_balance'], 2, ',', '.') }}
                        </td>
                    </tr>
                @endif
            </tbody>
