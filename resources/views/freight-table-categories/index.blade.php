...
@section('title', 'Categorias de Tabelas de Frete - TMS SaaS')
@section('page-title', 'Categorias de Tabelas de Frete')
...
<div class="page-header">
    ...
    <a href="{{ route('freight-table-categories.create') }}" class="btn-primary">
...
@else
    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(350px, 1fr)); gap: 20px;">
        @foreach($categories as $category)
            <div class="category-card" style="border-left-color: {{ $category->color ?? '#FF6B35' }};">
                    ...
                    <div class="category-title">
                        ...
                        <div>
                            ...
                            @if($category->description)
                                <div class="category-info">{{ Str::limit($category->description, 60) }}</div>
                            @endif
                        </div>
                    </div>
                    <div class="category-actions">
                        <a href="{{ route('freight-table-categories.edit', $category) }}" class="action-btn" title="Editar">
                            <i class="fas fa-edit"></i>
                        </a>
                        <form method="POST" action="{{ route('freight-table-categories.destroy', $category) }}" 
                ...
                <div style="margin-top: 15px; padding-top: 15px; border-top: 1px solid rgba(255,255,255,0.1);">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <span style="color: rgba(255,255,255,0.7);">
                            ...
                            {{ $category->active_freight_tables_count ?? 0 }} 
                            {{ $category->active_freight_tables_count == 1 ? 'tabela' : 'tabelas' }}
                        </span>
