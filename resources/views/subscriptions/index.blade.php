            ...
            <div class="current-subscription">
                ...
                <div class="subscription-info">
                    <div class="subscription-item">
                        <h4>Plano Atual</h4>
                        <p>{{ Auth::user()->tenant->currentSubscription()->plan->name }}</p>
                    </div>
                    <div class="subscription-item">
                        <h4>Status</h4>
                        <p>{{ ucfirst(Auth::user()->tenant->currentSubscription()->status) }}</p>
                    </div>
                    <div class="subscription-item">
                        <h4>Próxima Cobrança</h4>
                        <p>{{ Auth::user()->tenant->currentSubscription()->ends_at ? Auth::user()->tenant->currentSubscription()->ends_at->format('d/m/Y') : 'N/A' }}</p>
                    </div>
                </div>
            </div>
        @endif
            ...
            @foreach($plans as $plan)
                <div class="plan-card {{ $plan->is_popular ? 'popular' : '' }}">
                    ...
                    <div class="plan-price">
                        <span class="currency">R$</span>{{ number_format($plan->price, 0, ',', '.') }}
                    ...
                    <div class="plan-features">
                        ...
                        <ul class="feature-list">
                            @foreach($plan->features as $feature)
                                <li><i class="fas fa-check"></i> {{ ucfirst(str_replace('_', ' ', $feature)) }}</li>
                            @endforeach
                        </ul>
                    </div>
                    ...
                    <div class="plan-limits">
                        ...
                        @foreach($plan->limits as $limit => $value)
                            <div class="limit-item">
                                ...
                                <span>{{ is_numeric($value) ? number_format($value) : $value }}</span>
                            </div>
                        @endforeach
                    </div>

                    @if(!Auth::user()->tenant || !Auth::user()->tenant->currentSubscription())
                        <a href="{{ route('subscriptions.show', $plan) }}" class="btn-subscribe">
